...
function open(artid) {
    ...
    eId("art-" + artid).style.display = "block";
    if (["9-13", "9-20", "9-21"].includes(artid)) {
        eId("overlay-desc-toggle").style.visibility = "hidden";
    } else {
        eId("overlay-desc-toggle").style.visibility = "";
    }
    currentId = artid;
}
function toggleDesc() {
    ...
    if (!visible) {
        ...
        eId("desc-" + currentId).style.display = "block";
    } else {
        // alert('visible')
        eId("desc-container").style.display = "none";
